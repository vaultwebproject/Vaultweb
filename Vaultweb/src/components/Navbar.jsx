...
const Navbar = () => {
  ...
  const { userName, uuID, logout } = useContext(UserContext);
  const navigate = useNavigate();
  const isLoggedIn = !!uuID;
  ...
  return (
    ...
    <nav className="relative h-20 w-full px-4 sm:px-6 bg-gradient-to-b from-gray-900 to-black shadow-2xl border-b border-indigo-500/30 font-[Poppins] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto h-full flex items-center justify-between relative">
        {/* LOGO SECTION (Left) */}
        <Link to="/" className="flex items-center space-x-2 group cursor-pointer z-50">
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center space-x-10 text-xs font-bold text-gray-400 tracking-widest">
          ...
          <Link to="/create-org" className="hover:text-white transition-colors">
            ORGANISATION
          </Link>
        ...
        <div className="flex items-center space-x-5 z-50">
          {isLoggedIn ? (
            <>
              <span className="hidden md:block text-xs text-gray-400 font-mono truncate max-w-[180px]">
                {userName}
              </span>
              <button
                onClick={handleLogout}
                className="hidden md:block px-5 py-2 bg-gradient-to-r from-red-700 to-red-500 text-white text-[11px] rounded-full font-black hover:shadow-[0_0_20px_rgba(220,38,38,0.3)] transition-all active:scale-95 uppercase"
              >
                Sign Out
              </button>
            </>
      ...
      >
        <div className="flex flex-col items-center justify-center h-full space-y-8 text-2xl text-white font-medium">
          ...
          <Link to="/create-org" onClick={() => setIsOpen(false)}>
            Organisation
          </Link>
        </div>
      </div>
