...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-20 z-[100] bg-white/70 backdrop-blur-xl border-b border-sky-100/50 font-[Poppins] transition-all">
      <div className="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6 z-[110]">
          ...
          <button
            ...
            className="md:hidden p-2 text-slate-600 hover:bg-sky-50 rounded-lg transition-colors"
          >
      ...
      <div
        className={`fixed inset-0 bg-white/95 backdrop-blur-2xl transition-all duration-500 z-[105] md:hidden flex flex-col items-center justify-center space-y-8 ${
          isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none translate-y-4"
        }`}
      >
        <div className="flex flex-col items-center space-y-6 text-xl font-bold text-slate-900">
          <Link to="/" onClick={() => setIsOpen(false)} className="hover:text-sky-600">Home</Link>
          <Link to="/vault" onClick={() => setIsOpen(false)} className="hover:text-sky-600">Vaults</Link>
          <Link to="/create-org" onClick={() => setIsOpen(false)} className="hover:text-sky-600">Organisation</Link>
          <Link to="/admin" onClick={() => setIsOpen(false)} className="hover:text-sky-600">Admin</Link>
          
          <hr className="w-12 border-sky-100" />
